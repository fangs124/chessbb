...
pub use crate::transposition::{AtomicTranspositionTable, TranspositionTable, PositionData, NodeType, ATOMIC_TT_SIZE_MB};
...
impl ChessBoard {
    ...
    pub  fn parse_move(&self, move_str: &str) -> ChessMove {
        ...
        let (source, target) =  (Square::parse(&move_str[0..=1]), Square::parse(&move_str[2..=3]));
        /* castling-moves */
        match (source, target, matches!(self.core.mailbox[source.to_usize()], Some((_,PieceType::King)))) {
