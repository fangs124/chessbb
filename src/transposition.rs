...
impl AtomicTranspositionTable {
    ...
    pub(crate) fn load(&self, hash: ZobristHash, order: Ordering) -> Option<PositionData> {
        let data = self.data[hash].load(order);
...
impl Index<ZobristHash> for [SmallAtomicPositionData] {
    type Output = SmallAtomicPositionData;
...
impl IndexMut<ZobristHash> for [SmallAtomicPositionData] {
    #[inline(always)]
    fn index_mut(&mut self, index: ZobristHash) -> &mut Self::Output {
        return &mut self[index.to_index() % self.len()];
    }
...
impl SmallAtomicTranspositionTable {
    ...
    pub fn store(&self, hash: ZobristHash, depth: u16, eval: i16, ty: NodeType, best: Option<ChessMove>, order: Ordering) {
        ...
        self.data[hash].data.store(value, order);
        self.data[hash].hash.store(hash ^ cast(value), order);
    }

    #[inline(always)]
    pub(crate) fn load(&self, hash: ZobristHash, order: Ordering) -> Option<SmallPositionData> {
        let (tt_hash, value) = self.data[hash].load(order);
